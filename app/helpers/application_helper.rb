module ApplicationHelper
  ...
  def follow_button(user)
    label, key = current_user.following?(user) ? %w[Unfollow secondary] : %w[Follow primary]
    link_to(label, user_follow_path(@user), method: :post, remote: true, class: "btn btn-#{key} btn-sm")
  end

  def favorites_group_by_tag(books)
    books.map(&:tag_list).flatten.uniq.map do |tag| # Bookに登録しているタグを抽出、flattenで再帰的に配列、重複要素を除く
